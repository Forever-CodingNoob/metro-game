...
def show_station(station):
    number = int(request.args.get('number',
                                  '0'))  # get the number of problem of this station, if the number is None then it is set to 0
...
def sql_query_execute():
    ...
    print('sql:', repr(sql), ',', 'db_filename:', db_filename, ',', 'commands:', commands, ',', 'pretty-print:',
          prettyprint)
    ...
    return render_template('sql.html', logs=logs, output_in_html=output_in_html, db_names=DB_NAMES,
...
def start_auth(*, description, correct_keys, referer, method, setSession, session_key=None, correct_value=None,
               wrong_value=None):
